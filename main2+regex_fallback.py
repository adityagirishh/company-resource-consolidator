...
def validate_url(url: str) -> bool:
    ...
    try:
        # Parse URL to check if it's valid format
        parsed = urlparse(url)
...
def search_alternative_link(company_name: str, category: str) -> str:
    """Search for alternative valid links using Gemini"""
    try:
        search_prompt = f"""
        Find the most accurate and working URL for {company_name} {category}.
        Only return the direct URL, no explanations.
        ...
        if validate_url(suggested_url):
            return suggested_url
        return None
    except Exception as e:
        logging.error(f"Error searching alternative link: {e}")
        return None

def validate_and_fix_links(content: str) -> str:
    """Validate links and search for alternatives if invalid"""
    url_pattern = r'(https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+)'
    company_name_match = re.search(r'ORGANIZATION:\s*(.+)', content)
    company_name = company_name_match.group(1).strip() if company_name_match else ""
    
    def replace_with_valid_link(match):
        url = match.group(0)
        if validate_url(url):
            return url
            
        # Extract context to determine link category
        line = content[max(0, match.start()-100):min(len(content), match.end()+100)]
        categories = {
            "Base Site": "official website",
            "LinkedIn": "linkedin company page",
            "Glassdoor": "glassdoor company reviews",
            "GeeksForGeeks": "geeksforgeeks interview experiences",
            "LeetCode": "leetcode company questions",
            "Interview Questions": "interview questions",
            "System Design": "system design resources",
            "Data Structures": "data structures algorithms practice"
        }
        
        # Determine category based on context
        category = next((cat for key, cat in categories.items() if key.lower() in line.lower()), "")
        
        if category and company_name:
            alternative_url = search_alternative_link(company_name, category)
            if alternative_url:
                return alternative_url
                
        return f"{url} [⚠️ Invalid - No alternative found]"
    
    # Replace invalid links with alternatives
    updated_content = re.sub(url_pattern, replace_with_valid_link, content)
...
def generate_comprehensive_prompt(email_content):
    ...
    eg:
    1. Data Structures and Algorithms: https://www.geeksforgeeks.org/data-structures-and-algorithms/
    ...
    give the interview preparation resources in the following format:
    1. Data Structures and Algorithms: [Link to resource]
    2. System Design: [Link to resource]
    3. Behavioral Interviews: [Link to resource]
    4. Company Culture and Values: [Link to resource]
    ...
    issued for student interest by:
    Animesh Giri
    ...
    so make sure to verify the URLs before providing them.
    The output should be clean and concise, with no extra text or formatting.
...
def generate_shorts_script(company_info):
    ...
    try:
        ...
        **Script Blueprint:**
        ...
        [SLIDE 3: THE PAYCHECK]
        Announce the salary package. Be direct and exciting.
    ...
    if not gemini_api_key:
        ...
        with st.spinner("Processing email and generating company information..."):
            try:
                ...
                with st.spinner("Validating and fixing links..."):
                    validated_content = validate_and_fix_links(response.text)
                st.session_state.generated_info = validated_content
                
                st.success("✅ Company information generated successfully!")
                
                # Show warning for any remaining invalid links
                if "[⚠️ Invalid" in validated_content:
                    st.warning("⚠️ Some links could not be validated or replaced. Please verify manually.")
